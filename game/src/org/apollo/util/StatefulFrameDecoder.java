 ...
 * state is tracked by this class and is a user-specified enumeration. The state may be changed by calling the
 * {@link StatefulFrameDecoder#setState(Enum)} method. The current state is supplied as a parameter in the
 * {@link StatefulFrameDecoder#decode(ChannelHandlerContext, Channel, ChannelBuffer, Enum)} and
 * @param <T> The state enumeration.
 * {@link StatefulFrameDecoder#decodeLast(ChannelHandlerContext, Channel, ChannelBuffer, Enum)} methods. This class is
 * not thread safe: it is recommended that the state is only set in the decode methods overriden. {@code null} states
 * are not permitted. This means you cannot use {@link VoidEnum} like used in a {@link ReplayingDecoder}. If you do not
...
public abstract class StatefulFrameDecoder<T extends Enum<T>> extends FrameDecoder {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.jboss.netty.handler.codec.frame.FrameDecoder#decodeLast(org.jboss .netty.channel.ChannelHandlerContext,
	 * org.jboss.netty.channel.Channel, org.jboss.netty.buffer.ChannelBuffer)
	 */
	@Override
