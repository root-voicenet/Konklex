...
public class Allotments {
	...
	public boolean plantSeed(int objectX, int objectY, final int seedId) {
		...
		if (player.getInventory().getItemCount(allotmentData.getSeedId()) < allotmentData.getSeedAmount()) {
			player.getInterfaceSet().sendStatement(
					"You need atleast " + allotmentData.getSeedAmount() + " seeds to plant here.");
			return true;
		}
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (farmingStages[allotmentFieldsData.getAllotmentIndex()] == 0) {
			player.getInterfaceSet().sendStatement("This is an allotment patch. The soil has not been treated.",
	...
	public void handleFlowerProtection(int index) {
		...
		if (!farmingWatched[index]) {// Misc.random(100) <= maxChance) {
			...
			if (player.getFlowers().farmingState[indexGiven] != 3 && player.getFlowers().hasFullyGrown[indexGiven]
					&& player.getFlowers().farmingSeeds[indexGiven] == allotmentData.getFlowerProtect()) {
