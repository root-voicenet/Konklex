...
public final class NpcSynchronizationEventEncoder extends EventEncoder<NpcSynchronizationEvent> {
	...
	public GamePacket encode(NpcSynchronizationEvent event) {
		final GamePacketBuilder builder = new GamePacketBuilder(65, PacketType.VARIABLE_SHORT);
	...
	private void putAddCharacterUpdate(AddNpcSegment seg, NpcSynchronizationEvent event, GamePacketBuilder builder) {
		...
		final Position npc = seg.getPosition();
		final Position other = event.getPosition();
		...
		builder.putBits(1, 0);
		builder.putBits(12, seg.getNpcid()); // TODO 12
	 ...
	 * @param blockBuilder The builder.
	 */
	private void putTurnToEntityBlock(InteractingEntityBlock block, GamePacketBuilder blockBuilder) {
