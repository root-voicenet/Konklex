...
# A herb is an ingredient which needs identification before
# being used. This is done through implementation of the
# HerbloreMethod.
...
# An action which makes a player identify a herb.
class HerbIdentificationAction < Action
  ...
  def execute
    # Comment out this if-statement to disable level
	if @pulses == 0
	  if not check_skill character, @herb.level, "identify this herb"
	    stop
		return
	  end
	end
  ...
  def execute_action
	...
	if inventory.remove_slot(@slot, @item.id, 1) == 1
	  reward = @herb.item
		
	  inventory.add reward
	  player.skill_set.add_experience HERBLORE_ID, @herb.experience
	  player.send_message "You identify the herb as " + reward.definition.name + "."
	end
	stop
  end
...
GUAM_LEAF	= append_herb 249, 199,	3,	2.5
