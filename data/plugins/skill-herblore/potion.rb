...
# Represents an unfinished potion which can be invoked as
# a HerbloreMethod and used as an ingredient.
...
# Represents a finished potion which can be invoked as
# a HerbloreMethod.
...
# A DialogueAdapter used for mixing potions. It is also used
# as EnterAmountListener for the amount of mixing actions.
...
class UnfinishedMixingDialogueListener < MixingDialogueListener
  def calculate_maximum
	...
	amount = inventory.get_amount WATER_VIAL
	if amount <= 0
	  return 0
	end
...
class FinishedMixingDialogueListener < MixingDialogueListener
  def calculate_maximum
	...
	@action.potion.ingredients.each do |ingredient|
	  amt = inventory.get_amount ingredient.item.id
	  if amount > amt
	    amount = amt
	  end
	end
...
class MixingAction < Action
  ...
  def execute
    if @pulses == 0
	  if not @started
	    if not check_skill(character, @potion.level, @action)
		  stop
		  return
		end
		@started = true
	...
	character.play_animation MIXING_ANIM
	execute_action
...
class UnfinishedMixingAction < MixingAction
  ...
  def execute_action
	...
	player.send_message "You put the " + name + " in the water to make an unfinished " + name.sub(/ leaf$/, "") + " potion."
  ...
  def gather_materials
	...
	vial_slot = inventory.get_slot WATER_VIAL
	if vial_slot == -1
	  character.send_message "You do not have any more vials of water."
	  return false
	end

	item = @potion.herb.item
	herb_slot = inventory.get_slot item.id
	if herb_slot == -1
	  character.send_message "You do not have any more " + item.definition.name + "."
	  return false
	end
...
class FinishedMixingAction < MixingAction
  ...
  def execute_action
	...
	ingr = ItemDefinition.for_id(@ingredient).name.downcase
	name = @potion.item.definition.name.sub("(3)", "")

	player.send_message "You add the " + ingr + " to the mixture to make some " + name + "."
  ...
  def gather_materials
    ...
    vial_slot = inventory.get_slot @unfinished
	if vial_slot == -1
	  character.send_message "You do not have enough unfinished potions."
	  return false
	end
	
	ingredient_slot = inventory.get_slot @ingredient
	if ingredient_slot == -1
	  character.send_message "You do not have enough ingredients."
	  return false
	end
...
def append_unfinished_potion(item, herb, level)
  ...
  append_herblore_item potion, herb.item_id, WATER_VIAL
  return potion
end
