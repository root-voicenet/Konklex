...
class AlchemyAction < ItemSpellAction
  ...
  def illegal_item?
    case @item.id
	  when *ILLEGAL_ALC_ITEMS then true else false
	end
  end
  ...
  def execute_action
    ...
    if @pulses == 0
	  ...
	  reward = (item.definition.value * @spell.multiplier)
	  
	  inventory.remove_slot @slot, 1
	  inventory.add 995, reward
...
def append_alchemy(button, level, elements, high, animation, graphic, multiplier, experience, delay)
  alchemy = AlchemySpell.new(level, elements, high, animation, graphic, multiplier, experience,  delay)
  ALCHEMY_SPELLS[button] = alchemy
end
