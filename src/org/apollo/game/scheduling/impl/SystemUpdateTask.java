...
public class SystemUpdateTask extends ScheduledTask {
	...
	public void execute() {
		if (!started) {
			...
			Notification.getInstance().add("System Update", "System update is starting.", Type.NOTIFICATION);
			started = true;
		} else {
			for (Player player : World.getWorld().getPlayerRepository()) {
				player.logout();
			}
			Notification.getInstance().add("System Update", "System update is done.", Type.NOTIFICATION);
			stop();
		}
	}
}
