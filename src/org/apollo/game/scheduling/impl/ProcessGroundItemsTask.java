...
public final class ProcessGroundItemsTask extends ScheduledTask {
    ...
    @Override
    public void execute() {
	for (final Position position : instance.getItems().keySet()) {
	    final GroundItem item = instance.get(position);
	    if (item != null) {
		item.decreasePulses();
		if (item.getPulses() == instance.GLOBAL_PULSES)
		    for (final Player p : World.getWorld().getPlayerRepository())
			if (!p.getName().equals(item.getControllerName())
				&& p.getPosition().isWithinDistance(position, 60)) {
