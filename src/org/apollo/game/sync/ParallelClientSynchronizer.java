...
/**
 * An implementation of {@link ClientSynchronizer} which runs in a thread pool. A {@link Phaser} is used to ensure that the synchronization is complete, allowing control to return
 * to the {@link GameService} that started the synchronization. This class will scale well with machines that have multiple cores/processors. The
 * {@link SequentialClientSynchronizer} will work better on machines with a single core/processor, however, both classes will work.
 * @author Graham
 */
public final class ParallelClientSynchronizer extends ClientSynchronizer {
	...
	/**
	 * Creates the parallel client synchronizer backed by a thread pool with a number of threads equal to the number of processing cores available (this is found by the.
