...
public final class PlayerSynchronizationTask extends SynchronizationTask {
	...
	/**
	 * The maximum number of players to load per cycle. This prevents the update packet from becoming too large (the client uses a 5000 byte buffer) and also stops old spec PCs
	 * from crashing when they login or teleport.
	 */
	...
	public void run() {
		...
		PlayerSynchronizationEvent event = new PlayerSynchronizationEvent(player, lastKnownRegion, player.getPosition(), regionChanged, segment, oldLocalPlayers, segments);
		player.send(event);
	}
}
