...
public final class ShopItemActionHandler extends EventHandler<ItemActionEvent> {
	...
	private void buy(Player player, ItemActionEvent event) {
		Item item = new Item(event.getId(), optionToAmount(event.getOption()));
		if (event.getOption() == 1) {
			String name = player.getShop().getPayment().getName();
			int value = player.getShop().buyValue(item.getId());
			player.sendMessage((item.getDefinition().getName() != null ? item.getDefinition().getName() : "That") + ": currently costs " + value + " " + name + ".");
		} else {
	...
	private void sell(Player player, ItemActionEvent event) {
		...
		if (event.getOption() == 1) {
			String name = player.getShop().getPayment().getName();
			int value = player.getShop().sellValue(item.getId());
			if (value == -1) {
				player.sendMessage("You cannot sell items in this shop.");
			} else {
				player.sendMessage((item.getDefinition().getName() != null ? item.getDefinition().getName() : "That") + ": shop will buy that for " + value + " " + name + ".");
			}
		} else {
