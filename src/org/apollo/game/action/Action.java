...
/**
 * An action is a specialised {@link ScheduledTask} which is specific to a character. <p> <strong>ALL</strong> actions <strong>MUST</strong> implement the {@link #equals(Object)}
 * method. This is to check if two actions are identical: if they are, then the new action does not replace the old one (so spam/accidental clicking won't cancel your action, and
 * start another from scratch).
