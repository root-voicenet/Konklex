...
public class Inventory implements Cloneable {
	...
	public enum StackMode {
		/**
		 * When in {@link #STACK_ALWAYS} mode, an {@link Inventory} will stack every single item, regardless of the settings of individual items.
		 */
		STACK_ALWAYS,
		/**
		 * When in {@link #STACK_STACKABLE_ITEMS} mode, an {@link Inventory} will stack items depending on their settings.
		 */
	...
	/**
	 * Adds an item to this inventory. This will attempt to add as much of the item that is possible. If the item remains, it will be returned (in the case of stackable items, any
	 * quantity that remains in the stack is returned). If nothing remains, the method will return {@code null}. If something remains, the listener will also be notified which
	 * could be used, for example, to send a message to the player.
	...
	/**
	 * Creates a copy of this inventory. Listeners are not copied, they must be added again yourself! This is so cloned copies don't send updates to their counterparts.
	...
	public boolean contains(int id, int amount) {
		return contains(new Item(id, amount));
	}

	/**
	 * Checks if this inventory contains an item with the specified item.
	 * @param id The item.
	 * @return {@code true} if so, {@code false} if not.
	 */
	public boolean contains(Item id) {
		for (int i = 0; i < capacity; i++) {
			Item item = items[i];
			if (item != null && item.getId() == id.getId()) {
				return item.getAmount() >= id.getAmount();
			}
		}
		return false;
	}
	...
	/**
	 * Gets the amount of *item* the player has on his inventory.
	 ...
	 * @param id The id.
	 * @return The first found slot (starting at 0) containing the specified item, or {@code -1} if none of the slots matched the conditions.
	 */
	...
	public boolean hasRoomFor(Item item) {
		...
		for (int slot = 0; slot < capacity; slot++) {
			Item other = items[slot];
			if (stackable) {
				if (item != null && other.getId() == item.getId()) {
					long total = item.getAmount() + other.getAmount();
	...
	private boolean isStackable(ItemDefinition def) {
		if (mode == StackMode.STACK_ALWAYS) {
			...
			if (mode == StackMode.STACK_STACKABLE_ITEMS) {
	...
	/**
	 * Removes {@code amount} of the item with the specified {@code id}. If the item is stackable, it will remove it from the stack. If not, it'll remove {@code amount} items.
	 * @param id The id.
	 * @param amount The amount.
	...
	/**
	 * Removes {@code amount} of the item at the specified {@code slot}. If the item is not stacked, it will only remove the single item at the slot (meaning it will ignore any
	 * amount higher than 1). This means that this method will under no circumstances make any changes to other slots.
	 * @param slot The slot.
	...
	/**
	 * Removes {@code amount} of the item at the specified {@code slot}, under the condition that this item matches the specified {@code id}. If the item is not stacked, it will
	 * only remove the single item at the slot (meaning it will ignore any amount higher than 1). This means that this method will under no circumstances make any changes to other
	 * slots.
	 * @param slot The slot.
	 * @param id The item id.
	 * @param amount The amount to remove.
