...
public final class Inventory implements Cloneable, Iterable<Item> {
	...
	private final class InventoryIterator implements Iterator<Item> {
		...
		public Item next() {
			try {
				final int i = cursor;
				Item next = Inventory.this.get(i);
				lastRet = i;
				cursor = i + 1;
				return next;
		...
		if (stackable) {
			for (int slot = 0; slot < capacity; slot++) {
				...
				if (other != null && other.getId() == id) {
					long total = (long) item.getAmount() + (long) other.getAmount();
					int amount;
					int remaining;
	...
	public boolean addAll(Inventory inventory) {
		...
		boolean oldFiringEvents = firingEvents;
