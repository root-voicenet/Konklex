...
import org.apollo.game.model.inter.pouch.MoneyPouch;
...
import org.apollo.game.model.skill.HiscoreSaver;
...
public final class Player extends Character {
	 ...
	 */
	private MoneyPouch moneyPouch = new MoneyPouch(this);
	...
	public void exitInitialEvents() {
		...
		Notification.getInstance().add("Player unregistered", getName() + " has logged out.", Type.NOTIFICATION);
		World.getWorld().getMessaging().deregister(this);
		try {
			HiscoreSaver.save(this);
		} catch (Exception e) {
		}
	}
	...
	private void initSkills() {
		...
		// prayer listener
		SkillListener prayerListener = new PrayerSkillListener(this);
		// hitpoints listener
		SkillListener hitpointListener = new HitpointSkillListener(this);
		// add the listeners
	...
	public void send(org.apollo.game.event.Event event) {
		if (isActive()) {
			if (!queuedEvents.isEmpty()) {
				for (org.apollo.game.event.Event queuedEvent : queuedEvents) {
	...
	private void sendInitialEvents() {
		// vital initial stuff
		send(new IdAssignmentEvent(getIndex(), members)); // TODO Should this be sent on a reconnect?
	...
	/*
	 * (non-Javadoc)
	 * @see org.apollo.game.model.Character#teleport(org.apollo.game.model.Position)
	 */
	@Override
