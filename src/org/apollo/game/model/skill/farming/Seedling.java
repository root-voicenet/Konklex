...
public class Seedling {
	...
	public boolean waterSeedling(int itemUsed, int usedWith, int itemUsedSlot, int usedWithSlot) {
		...
		player.sendMessage("You water the " + new Item(seedlingData.getSeedId()).getDefinition().getName().toLowerCase() + ".");
	...
	public boolean placeSeedInPot(int itemUsed, int usedWith, int itemUsedSlot, int usedWithSlot) {
		...
		if (seedlingData == null || (itemUsed != 5354 && usedWith != 5354))
			return false;
		if (player.getInventory().removeSlot(itemUsed == 5354 ? itemUsedSlot : usedWithSlot, new Item(seedlingData.getSeedId())) > 0) {
			player.getInventory().set(itemUsed == 5354 ? itemUsedSlot : usedWithSlot, new Item(seedlingData.getUnwateredSeedlingId()));
	...
	public boolean fillPotWithSoil(int itemId, int objectX, int objectY) {
		if (itemId != 5350)
			return false;
		if (!(player.getAllotment().checkIfRaked(objectX, objectY) || player.getBushes().checkIfRaked(objectX, objectY) || player.getFlowers().checkIfRaked(objectX, objectY) || player.getFruitTrees().checkIfRaked(objectX, objectY) || player.getHerbs().checkIfRaked(objectX, objectY) || player.getHops().checkIfRaked(objectX, objectY) || player.getSpecialPlantOne().checkIfRaked(objectX, objectY) || player.getSpecialPlantTwo().checkIfRaked(objectX, objectY))) {
