...
public final class SkillSet {
	...
	public int getCombatLevel() {
		...
		if (melee >= ranger && melee >= mage) {
			...
			if (ranger >= melee && ranger >= mage) {
				combat += ranger;
			} else
				if (mage >= melee && mage >= ranger) {
					combat += mage;
				}
	...
	public void increaseSkill(int id, int amount) {
		setSkill(id, new Skill(getSkill(id).getExperience(), getSkill(id).getCurrentLevel() + amount, getSkill(id).getMaximumLevel()));
	}
