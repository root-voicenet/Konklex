...
public class GroundItem {
	...
	if (stackable) {
	    final GroundItem groundItem = new GroundItem(p instanceof Player ? ((Player) p).getName() : "", item, pos);
	    p.send(new PositionEvent(p.getPosition(), pos));
	    p.send(new GroundItemEvent(itemId, itemAmount));
	    updateMap(groundItem, item);
	} else
	    for (int i = 0; i < itemAmount; i++) {
		final GroundItem groundItem = new GroundItem(p instanceof Player ? ((Player) p).getName() : "",
			new Item(itemId, 1), pos);
		p.send(new PositionEvent(p.getPosition(), pos));
	...
	final int findx = p.getX() - 8 * p.getTopLeftRegionX();
	final int findy = p.getY() - 8 * p.getTopLeftRegionY();
	final Collection<List<GroundItem>> collection = getItems().values();
	final Iterator<List<GroundItem>> iterator = collection.iterator();
	while (iterator.hasNext()) {
	    final List<GroundItem> list = iterator.next();
	    if (list == null)
		return null;
	    for (int i = 0; i < list.size(); i++) {
		final GroundItem g = list.get(i);
		final int lookx = g.getPosition().getX() - 8 * g.getPosition().getTopLeftRegionX();
		final int looky = g.getPosition().getY() - 8 * g.getPosition().getTopLeftRegionY();
	...
	if (p.getInventory().freeSlots() <= 0)
	    ...
	    final GroundItem item = get(position);
	    if (item != null)
