...
public final class RegionManager {
		...
		for (final GroundItem groundItem : collection)
			if (groundItem.getPosition().equals(position) && player.getPosition().equals(groundItem.getPosition()))
				if (groundItem.getItem().getId() == item)
					if (groundItem.getControllerName().equals("")
	 ...
	 */
	public Map<Region, List<Event>> getLocalEvents(Character character) {
		final Map<Region, List<Event>> localEvents = new HashMap<Region, List<Event>>();
		final Region[] regions = getSurroundingRegions(character.getPosition());
		final int distance = Position.MAX_DISTANCE;
		for (final Region region : regions) {
			final List<Event> events = new ArrayList<Event>();
			for (final Event event : region.getEvents())
				if (event.getEventId() == 1 || event.getEventId() == 2) {
					final MapEvent map = (MapEvent) event;
					final Position position = map.getPosition();
					if (position.getHeight() == character.getPosition().getHeight()) {
						if (position.getDistance(character.getPosition()) <= distance) {
							events.add(event);
						}
					}
				} else {
			...
			if (events.size() > 0)
				localEvents.put(region, events);
		}
		return Collections.unmodifiableMap(localEvents);
	}
	...
	public void removeRegion(Region region) {
		activeRegions.remove(region.getCoordinates());
	}
	...
	public Region[] getSurroundingRegions(Position location) {
		...
		final Region[] surrounding = new Region[17];
		...
		surrounding[9] = getRegion(regionX - 2, regionY - 2);
		surrounding[10] = getRegion(regionX + 2, regionY + 2);
		surrounding[11] = getRegion(regionX - 2, regionY);
		surrounding[12] = getRegion(regionX, regionY - 2);
		surrounding[13] = getRegion(regionX + 2, regionY);
		surrounding[14] = getRegion(regionX, regionY + 2);
		surrounding[15] = getRegion(regionX - 2, regionY + 2);
