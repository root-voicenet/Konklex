...
public final class RegionMusic {
	...
	private static Music getMusicId(Player player) {
		...
		for (int i = 0; i < songs.length; i++) {
			if (x >= songs[i].getSwX() && x <= songs[i].getNeX() && y >= songs[i].getSwY() && y <= songs[i].getNeY()) {
				return songs[i];
			}
		}
		return null;
	...
	private static final Music[] songs = {
		...
		// new Music(Id, 2972, 3103, 3014, 3136), // Mudskipper Melody MISSING
		...
		// new Music(Id, 3010, 3904, 3076, 3969), // Pirates of Peril MISSING
