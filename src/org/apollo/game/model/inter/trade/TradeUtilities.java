...
public final class TradeUtilities {
	...
	private static void initTrade(Player player, Player acquaintance) {
		...
		player.getInterfaceSet().openWindowWithSidebar(new TradeInterfaceListener(s), TradeConstants.TRADE_INTERFACE_ID, TradeConstants.INVENTORY_INTERFACE_ID);
	...
	public static void sendTradeRequest(Player player, Player acquaintance) {
		if (!World.getWorld().isPlayerOnline(acquaintance.getName()) || !player.getPosition().isWithinDistance(acquaintance.getPosition(), 20)) {
