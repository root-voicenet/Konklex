...
public final class TradeUtilities {
	...
	private static void initTrade(Player player, Player acquaintance) {
		...
		final TradeSession s = player.setTradeSession(new TradeSession(player,
				acquaintance));
		player.getInterfaceSet().openWindowWithSidebar(
				new TradeInterfaceListener(s),
				TradeConstants.TRADE_INTERFACE_ID,
				TradeConstants.INVENTORY_INTERFACE_ID);
		player.send(new SetInterfaceTextEvent(3417, "Trading with: "
				+ TextUtil.capitalize(acquaintance.getName())));
	...
	public static void sendTradeRequest(Player player, Player acquaintance) {
		if (!World.getWorld().isPlayerOnline(acquaintance.getName())
				|| !player.getPosition().isWithinDistance(
						acquaintance.getPosition(), 20)) {
			player.sendMessage("Could not find the player.");
			return;
		}
		final Request ar = acquaintance.getRequest();
		if (ar != null && ar.getType() == Request.Type.TRADE_REQUEST
				&& ar.getRecipient() == player)
		...
		final Request r = new Request(player, acquaintance,
				Request.Type.TRADE_REQUEST, Request.State.PENDING);
		player.setRequest(r);
		player.sendMessage("Sending trade request...");
		acquaintance.sendMessage(TextUtil.capitalize(player.getName())
				+ ":tradereq:");
	}
