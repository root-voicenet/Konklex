...
public final class World {
	...
	public void init(int release, IndexedFileSystem fs, PluginManager mgr, ServerContext context) throws IOException {
		...
		for (final GameObject go : objectSpawns) {
			if (go.isDelete()) {
				regionManager.getRegionByLocation(go.getLocation()).addEvent(new DestroyObjectEvent(go));
				if (!go.isReplace()) {
					continue;
				}
			}
			register(go);
			// TODO When a object is added, removed the old static object.
		}
		is.close();
	...
	public void register(Minigame minigame) {
		MinigameService service = context.getService(MinigameService.class);
		if (service != null) {
			service.addMinigame(minigame);
		}
	}
	...
	public RegistrationStatus register(final Player player) {
		...
		if (SystemUpdateTask.isUpdating()) {
			...
			if (success) {
				...
				if (service != null) {
					service.sendAll(new SendPlayerMethod(player.getEncodedName(), player.getPrivilegeLevel().toInteger(), true));
				}
	...
	public void unregister(GameObject object) {
		final GameObject replace = getObject(object.getLocation());
		synchronized (this) {
			if (objects.remove(replace)) {
				regionManager.getRegionByLocation(object.getLocation()).removeObject(replace);
			}
		}
	}
