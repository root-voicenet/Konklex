...
/**
 * Represents the set of interfaces the player has open. <p> This class manages all six distinct types of interface (the last two are not present on 317 servers). </p> <ul>
 * <li><strong>Windows:</strong> the ones people mostly associate with the word interfaces. Things like your bank, the wildy warning screen, the trade screen, etc.</li>
 * <li><strong>Overlays:</strong> display in the same place as windows, but don't prevent you from moving. For example, the wilderness level indicator.</li>
 * <li><strong>Dialogues:</strong> interfaces which are displayed over the chat box.</li> <li><strong>Sidebars:</strong> an interface which displays over the inventory area.</li>
 * <li><strong>Fullscreen windows:</strong> a window which displays over the whole screen e.g. the 377 welcome screen.</li> <li><strong>Fullscreen background:</strong> an interface
 * displayed behind the fullscreen window, typically a blank, black screen.</li> </ul>
 * @author Graham
 */
public final class InterfaceSet {
	...
	public InterfaceListener removeListener() {
		InterfaceListener listener = this.listener;
		this.listener = null;
		return listener;
	}
