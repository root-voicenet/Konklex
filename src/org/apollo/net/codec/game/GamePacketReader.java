		...
		if (order == DataOrder.BIG) {
				...
				for (int i = length - 1; i >= 0; i--) {
					if (i == 0 && transformation != DataTransformation.NONE) {
						if (transformation == DataTransformation.ADD) {
							...
							if (transformation == DataTransformation.NEGATE) {
								longValue |= (-buffer.readByte()) & 0xFF;
							} else
								if (transformation == DataTransformation.SUBTRACT) {
			...
			if (order == DataOrder.LITTLE) {
				...
				if (order == DataOrder.MIDDLE) {
					...
					if (order == DataOrder.INVERSED_MIDDLE) {
						if (transformation != DataTransformation.NONE) {
