...
public final class SwitchItemEventDecoder extends EventDecoder<SwitchItemEvent> {
	...
	public SwitchItemEvent decode(GamePacket packet) {
		...
		final int newSlot = (int) reader.getUnsigned(DataType.SHORT,
				DataOrder.LITTLE, DataTransformation.ADD);
		final boolean inserting = reader.getUnsigned(DataType.BYTE,
				DataTransformation.ADD) == 1;
		final int interfaceId = (int) reader.getUnsigned(DataType.SHORT,
				DataTransformation.ADD);
		final int oldSlot = (int) reader.getUnsigned(DataType.SHORT,
				DataOrder.LITTLE);
