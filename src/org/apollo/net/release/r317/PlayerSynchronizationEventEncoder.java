...
public final class PlayerSynchronizationEventEncoder extends EventEncoder<PlayerSynchronizationEvent> {
	...
	private void putForceMovementBlock(ForceMovementBlock block, GamePacketBuilder blockBuilder) {
		final Position current = block.getCurrentPosition();
		final Position reachable = block.getPosition();
		blockBuilder.put(DataType.BYTE, DataTransformation.NEGATE, current.getX());
		blockBuilder.put(DataType.BYTE, DataTransformation.SUBTRACT, current.getY());
		blockBuilder.put(DataType.BYTE, DataTransformation.SUBTRACT, reachable.getX());
		blockBuilder.put(DataType.BYTE, DataTransformation.NEGATE, reachable.getY());
		blockBuilder.put(DataType.SHORT, block.getFirstSpeed());
		blockBuilder.put(DataType.SHORT, DataTransformation.ADD, block.getSecondSpeed());
		blockBuilder.put(DataType.BYTE, block.getDirection());
	}
	...
	private void putTurnToPositionBlock(TurnToPositionBlock block, GamePacketBuilder blockBuilder) {
		final Position pos = block.getPosition();
