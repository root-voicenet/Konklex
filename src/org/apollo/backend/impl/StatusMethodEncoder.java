...
public final class StatusMethodEncoder extends MethodEncoder<StatusMethod> {
	...
	public FrontendPacket encode(StatusMethod method) {
		final FrontendPacketBuilder builder = new FrontendPacketBuilder(
				"getStatus");
		final String status = SystemUpdateTask.isUpdating() ? "update"
				: Config.SERVER_WHITELIST ? "down" : "up";
		builder.addParameter("status", status);
		builder.addParameter("users", World.getWorld().getPlayerRepository()
				.size());
