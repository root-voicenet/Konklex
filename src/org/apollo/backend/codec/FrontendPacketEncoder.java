...
public final class FrontendPacketEncoder {
	...
	private void encode(FrontendPacket packet) {
		...
		if (!getAttribute(1)) {
			...
			if (!isStreaming) {
				...
				resp.setHeader(Names.TRANSFER_ENCODING, Values.CHUNKED);
			}
			channel.write(resp);
			setAttribute(1, true);
		}
	...
	private void writeStream(String encodedString) {
		final ChannelBuffer buffer = ChannelBuffers.wrappedBuffer(encodedString.getBytes(CharsetUtil.UTF_8));
		final HttpChunk chunk = new DefaultHttpChunk(buffer);
		channel.write(chunk);
	}
}
