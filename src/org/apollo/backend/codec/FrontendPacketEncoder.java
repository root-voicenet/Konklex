...
public final class FrontendPacketEncoder {
	...
	private String addError(FrontendPacket packet) {
		JSONObject object = packet.getParameters();
		try {
	...
	public <E extends Method> void encode(E method) {
		MethodEncoder<E> encoder = (MethodEncoder<E>) Frontend.getInstance().getMethodEncoder(method.getClass());
		if (encoder != null) {
			FrontendPacket packet = encoder.encode(method);
			encode(packet);
			if (getAttribute(2)) {
	...
	private void encode(FrontendPacket packet) {
		String encodedString = !getAttribute(3) ? packet.getParameters().toString() : addError(packet);
		boolean isStreaming = getAttribute(2);
		if (!getAttribute(1)) {
			...
			if (!isStreaming) {
				ChannelBuffer wrappedBuf = ChannelBuffers.wrappedBuffer(encodedString.getBytes());
	...
	private void writeStream(String encodedString) {
		try {
			byte[] toWrite = (encodedString.trim() + "\r\n").getBytes("UTF-8");
			ChannelBuffer buf = ChannelBuffers.buffer(toWrite.length);
