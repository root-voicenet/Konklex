...
public final class FrontendPacketDecoder {
	...
	private <E extends Method> E decode(FrontendPacket packet) {
		...
		final MethodDecoder<E> decoder = (MethodDecoder<E>) Frontend
				.getInstance().getMethodDecoder(reader.getMethod());
		if (decoder != null) {
			try {
	...
	public <E extends Method> E decode(Map<String, List<String>> parameters) {
		...
		for (final Entry<String, List<String>> kv : parameters.entrySet()) {
			try {
				object.put(kv.getKey(),
						kv.getValue().toString().replaceFirst("\\[", "")
						.replaceFirst("\\]", ""));
	...
	public <E extends Method> E decode(String uri) {
		boolean error = false;
		final JSONObject object = new JSONObject();
		final Map<String, List<Object>> parameters = TextUtil
				.getUrlParameters(uri);
