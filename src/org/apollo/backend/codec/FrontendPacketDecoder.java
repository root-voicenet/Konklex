...
public final class FrontendPacketDecoder {
	...
	private <E extends Method> E decode(FrontendPacket packet) {
		...
		MethodDecoder<E> decoder = (MethodDecoder<E>) Frontend.getInstance().getMethodDecoder(reader.getMethod());
		if (decoder != null) {
			try {
	...
	public <E extends Method> E decode(String uri) {
		...
		Map<String, List<Object>> parameters = TextUtil.getUrlParameters(uri);
		for (Entry<String, List<Object>> kv : parameters.entrySet()) {
			try {
