...
/**
 * A file system based on top of the operating system's file system. It consists of a data file and index files. Index files point to blocks in the data file, which contains the
 * actual data.
 * @author Graham
 */
public final class IndexedFileSystem implements Closeable {
	...
	private void detectLayout(File base) throws Exception {
		...
		if (oldEngineData.exists() && !oldEngineData.isDirectory())
			...
			if (newEngineData.exists() && !oldEngineData.isDirectory())
