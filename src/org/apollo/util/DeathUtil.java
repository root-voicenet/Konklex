...
public class DeathUtil {
	...
	final Inventory keep = new Inventory(size + 1);
	final Map<Item, Integer> top = top(player);
	int count = 0;
	for (final Item item : top.keySet())
	    if (!(count >= size)) {
		keep.add(item);
		count++;
	    }
	...
	final Map<Item, Integer> list = new HashMap<Item, Integer>();
	...
	for (final Item item : player.getEquipment().getItems())
	    if (item != null)
		list.put(item, item.getDefinition().getValue());
	final ValueComparator bvc = new ValueComparator(list);
	final TreeMap<Item, Integer> sorted_list = new TreeMap<Item, Integer>(bvc);
