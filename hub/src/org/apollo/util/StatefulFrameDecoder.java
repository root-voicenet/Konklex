 ...
 * used by other classes. The current state is tracked by this class and is a
 * user-specified enumeration. The state may be changed by calling the
 * {@link StatefulFrameDecoder#setState(Enum)} method. The current state is
 * supplied as a parameter in the
 ...
 * only set in the decode methods overriden. {@code null} states are not
 * permitted. This means you cannot use {@link VoidEnum} like used in a
 * {@link ReplayingDecoder}. If you do not need state management, the
 * {@link FrameDecoder} class should be used instead.
 * @author Graham
 */
